...
- **Follow the shape** (75% by default) spends the quads where the form needs them. A flat quad of edge h on a surface bending with curvature κ misses it by about κh²/8, so for an even error the number of quads per area follows curvature: small quads on chains, jewellery, faces and strand edges, big ones on flat skirts and backs. Curvature is measured once per model between neighbouring patches about 0.4% of its size (whose averaged normals ignore surface noise), kept within 1/3 to 12 times its median and smoothed so quad sizes change gradually. Higher values keep small forms better; every change of size costs some poles, from about one vertex in eight at 0% to one in three at 100%. On a character's necklace at 21k quads, 0% loses it entirely and 100% keeps the ring as a closed band.
- Triangles mode does this by itself: the simplifier always takes the collapse that moves the surface least, so flat areas merge into large triangles first and small forms keep theirs. For crisp small hard-surface details at a low count, Triangles mode is still the better choice.
